...
class m221226_013514_create_staff_table extends Migration
{
    ...
    public function safeUp()
    {
        $this->createTable('{{%staff}}', [
            ...
            'honorific' => $this->string(10)->notNull()->comment('Title/Honorific'),
            'full_name' => $this->string(50)->notNull()->comment('Full Name'),
            'staff_extension' => $this->string(5)->null()->comment('Staff Extension'),
            'country_code' => $this->string(4)->unique()->notNull()->comment('Country Code'),
            'phone_prefix' => $this->integer(3)->unique()->null()->comment('Phone Prefix'),
            'phone_number' => $this->integer(9)->unique()->notNull()->comment('Phone Number'),
            'fk_department' => $this->integer()->unsigned()->notNull()->comment('Department ID'),
            'fk_position' => $this->integer()->unsigned()->notNull()->comment('Job Position'),
            'fk_office' => $this->integer()->unsigned()->notNull()->comment('Office ID'),
            'gender' => $this->string(20)->notNull()->comment('Gender'),
